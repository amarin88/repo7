...
router.get("/current", (req, res) => {
  try {
    ...
    const checkToken = verifyToken(token); //Verifica que el token sea válido
    if (!checkToken)
      return res
        .status(403)
        .json({ status: "error", response: "Invalid token" }); //En el caso que la validación falle retornamos error

    return res.status(200).json({ status: "success", payload: checkToken }); //En el caso de que el token sea válido devolvemos el token
  } catch (error) {
    console.log(error);
    res.status(500).json({
      status: error.status || 500, // Devuelve el código de estado del error o 500 si no está definido
